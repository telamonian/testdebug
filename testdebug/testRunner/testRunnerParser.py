...
class TestRunnerParser(Parser):
    def genDefaultArgGroups(self):
        # group of args relating to discovery of tests
        testDiscoveryArgs = ArgGroup('testDiscoveryArgs',
            ArgProperty('fullname', nargs='*', type=str, help='Python module names for test auto-discovery. Packages will be recursively searched, and all contained TestBases will be imported.'),
            ArgProperty('-p', '--path', nargs='*', type=Path, help='File system path for test auto-discovery. Path will be recursively searched, and all contained TestBases will be imported.'),
        )
        ...
        testExecArgs = ArgGroup('testExecArgs',
            ...
            ArgProperty('-r', '--profile', action='store_true', help='Setting this flag will cause all tests to be profiled. Only works when running with the default testing framework.'),
        )

        # group of args relating to test frameworks
        ...
        # group of args that control how error messages are displayed
